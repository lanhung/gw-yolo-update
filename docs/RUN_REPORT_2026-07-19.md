...
同一条嵌套曲线现已扩展到 500/1k/2k/5k，validation mean IoU 分别为
0.719/0.787/0.816/0.873；到 5k 仍未平台化。内存缓存保持 72 点指标逐位一致，并把耗时从
157.5 秒降到 27.6 秒。六点幂律拟合给出 α≈0.527、探索性 10k≈0.877，但仅有一个 seed，
因此已经启动实际 10k 点，不把外推当结论。

真实数据链路已完成 H1 半网验收：GW231123_135430 的 O4a 4 kHz HDF5 为 129,057,388 bytes，
SHA256 `da6eb3cc48e2a0abb41ec31f70a2ecc3e425bc3c907c77e6ccc6d99794a59671`；读取 64 秒上下文的
DQmask=511、Injmask=23，生成 H1×3Q×96×96 有限张量。L1 对应文件仍在续传，因此这不是
多探测器结果。

Gravity Spy H1_O1 官方 CSV 也已通过 Zenodo MD5，形成 1,391 个独立高置信 glitch ID、1,138
个 64 秒 GPS block、19 类的真实锚点索引。它与旧训练集中仅 48 个 glitch-only 训练组形成
数量级差距，并再次支持大幅扩充真实物理样本。

H1 O3a 也已通过发布方 MD5：80,763 原始行中有 59,857 个合格高置信非 chirp glitch，分层
